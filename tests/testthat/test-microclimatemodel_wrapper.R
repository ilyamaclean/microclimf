test_that("The full microclimate model runs on a 24h test case and returns expected outputs", {
  ...
  zen <- 0; for (hr in 0:23) zen[hr+1] <- solpositionCpp(50, -5, 2024, 3, 21, hr)[1]
  si <- cos(zen * pi/180); si[si < 0] <- 0
  Rdir <- 0.3 * csr * si
  SWd <- 0.3 * csr                                         # W m-2
  Rdif <- SWd - Rdir                                       # W m-2
  LWd    <- rep(350, n)                                    # W m-2
  ...
  vegp <- c(
    ...
    ltra=0.2, leafd=0.05, gsmax=0.33
  )
  # groundp
  groundp <- c(
    gref=0.15, slope=0, aspect=180, rho=1.53, Vm=0.509, Vq=0.06, Mc=0.5422,
    Smin=0.091, Smax=0.419
  )
  # Run below canopy microclimate model
  out = microclimatemodel_wrapper(obstime, climdata, vegp, groundp,
  ...
  if (!is.null(Tleaf_dif)) { expect_lte(max(Tair_dif), 5) }
  if (!is.null(rh)) { expect_gte(min(rh), min(RH) - 5); expect_lte(max(rh), 100) }
  if (!is.null(uz_rat)) { expect_gte(min(uz_rat), 0.084); expect_lte(max(uz_rat), 0.087) }
  ...
  if (!is.null(Rsw_rat)) { expect_gte(min(Rsw_rat), 0.10); expect_lte(max(Rsw_rat), 0.12) }
  if (!is.null(Rlw_rat1)) { expect_gte(min(Rlw_rat1), 0.94); expect_lte(max(Rlw_rat1), 1.1) }
  if (!is.null(Rlw_rat2)) { expect_gte(min(Rlw_rat2), 0.94); expect_lte(max(Rlw_rat2), 1.15) }
  ...
  Rlw_rat2 <- Rlwup / LWd
  # ---- Bounds / sanity checks ----
  if (!is.null(Tz_dif)) { expect_lte(max(Tz_dif), 10) }
  if (!is.null(soilm)) { expect_gte(min(soilm), 0.299); expect_lte(max(soilm), 0.301) }
  if (!is.null(Rdir_rat)) { expect_gte(min(Rdir_rat), 0); expect_lte(max(Rdir_rat), 0.25) }
  if (!is.null(Rdif_rat)) { expect_gte(min(Rdif_rat), 0.23); expect_lte(max(Rdif_rat), 0.25) }
  if (!is.null(Rsw_rat)) { expect_gte(min(Rsw_rat), 0.12); expect_lte(max(Rsw_rat), 0.15) }
  ...
  # Lengths should match the 24 h input where applicable
  check_len <- function(x) if (!is.null(x)) expect_equal(length(x), n)
  lapply(list(Tz,soilm))
  if (!is.null(Tz)) { expect_gte(min(Tz), min(Tair)-1); expect_lte(max(Tz), max(Tair)+1) }
  if (!is.null(soilm)) { expect_gte(min(soilm), 0.299); expect_lte(max(soilm), 0.301) }
})

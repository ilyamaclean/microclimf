...
<small> 
**Fig. S2.1**. Effects of diabatic coefficients on the estimation of friction velocity. Here the iterative point microclimate model is run for a reference location with a plant area index of 3 and vegetation height of 1 m and for a target location with a plant area index of 1 and vegetation height of 0.5 m over the course of an entire year using the inbuilt weather datasets for 2017 from Caerthillian Cove in Cornwall. In (a) a frequency histogram of the diabatic coefficients for the target location are shown. In (b) the vegetation-driven determinant of friction velocity $(\ln((2-d)/z_m)+\psi_m)$ for the target location is plotted over the course of the year, with the red line depicting $\psi_m=0$. In (c) the log of friction velocity for the target location with and without a diabatic correction applied are plotted against one another, demonstrating that friction velocity is under-estimated when wind speeds are low. In (d) the estimated friction velocity using the ratio approach described above (x-axis) is plotted against the true friction velocity calculated by running the point microclimate iteratively (y-axis). The ratio approach significantly improves estimates of friction velocity when wind speeds are low.
</small>
...
The stomatal resistance of individual leaves is modelled using a simplified formulation derived from the Cowan & Farquhar (1977) photosynthesis framework, following Ellers et al. (2020). Rather than implementing the full 26-parameter model and solving it iteratively, the essential behaviour of the model is distilled into a small number of explicit functions. These capture the reduction in stomatal conductance under soil moisture limitation and its dependence on photosynthetically active radiation, while retaining the response characteristics of the full model. Thus

$$\frac{\tilde\rho}{r_s}=\min\left(g_{PAR},g_{\psi_w}\right)$$
where $\small g_{PAR}$ is the stomatal response to absorbed photosynthetically active radiation given by

$$ g_{PAR}=g_{mx}\times2^{-\left(R_{mx}^{PAR}-R_{abs}^{PAR}/0.2R_{mx}^{PAR}\right)}$$

where $\small R_{abs}^{PAR}$ is the flux density of absorbed PAR constrained such that $\small R_{abs}^{PAR}\le R_{mx}^{PAR}$ and $\small R_{mx}^{PAR}$ is the threshold value of $\small R_{abs}^{PAR}$ above which $g_{PAR}$ no longer increases. The term $\small g_{\psi_w}$ characterizes the drought response and is given by

$$g_{\psi_w}=g_{mx}\left(1-\frac{\exp\left(-\kappa\psi_w\right)-1}{\exp\left(-\kappa\psi_0\right)-1}\right)$$
where $\small \psi_w$ is water potential constrained such that $\psi_w\ge\psi_0$ where $\psi_0$ is the threshold value of $\psi_w$ at which $\small g_{\psi_w}$ becomes zero. Water potential is derived from the volumetric soil water fraction $\small \theta$ as

$$\psi_w=0.01\psi_e\left(\frac{\theta}{\theta_{mx}}\right)^{-b}$$
where $\small \theta_{mx}$ is the volumetric water fraction at saturation, $\small \psi_e$ air entry potential and $\small b$ a shape parameter both derived form soil type following Campbell (1986). Here
$\small g_{mx}$ is a user provided maximum stomatal conductance, provided as molar conductance ($\small mol·m^{-2}·s$) to ensure conformance with Körner (1995) who gives values for major vegetation types of the globe. The `vegpfromhab` function built into the package derives these for user-specified vegetation types. The coefficients $\small \kappa$, $\small \psi_0$ and $\small R_{mx}^{PAR}$ are specific to vegetation type and inferred from vegetation height and the leaf angle inclination parameter $\small x$. 

To a reasonable approximation (Raupach 1994; Kelliher *et al* 1995), $\small r_c$  is the inverse parallel sum of the stomatal resistances of individual leaves such that

$$\frac{\tilde\rho}{r_c}=\frac{\tilde\rho}{g_{sun}L_{sun}+g_{shade}L_{shade}}$$
where $\small g_{sun}$ the stomatal conductance of $\small g_{shade}$ of shaded leaves and $\small L_{sun}$ and $\small L_{shade}$ the total one-sided leaf area of sunlit and shaded leaves respectively. $g_{shade}$ is the stomatal response with $\small R_{abs}^{PAR}$ set to an average shade value for the canopy given by

$$ R_{abs}^{shade}=\left(1-\omega_{PAR}\right)R^d_0\frac{1-\exp(-P_{AI})}{P_{AI}}$$
and

$$R_{abs}^{sun}=R^b_0k\cos Z\left(1-\omega_{PAR}\right)+R^{shade}_{abs}$$
where $\small \omega_{PAR}$ is approximated by $\small 0.5\left(\alpha_L+\tau_L\right)$ (0.5 times leaf reflectance + transmittance). The sunlit and shaded leaf areas are given by

$$L_{sun}=P_{AI}-L_{shade}=\frac{1-\exp(-kP_{AI})}{k}$$
...
<small>
**Table S2.1**. Soil properties for given soil types. This table is stored as an internal dataset in microclimf. Here $\rho_d$ is the bulk density of the soil, $V_Q$ and $V_M$ volumetric quartz and mineral content respectively, $M_C$ the mass fraction of clay, $\theta_{mx}$ and $\theta_{mn}$ the saturated and residual soil moisture fraction, $K_{sat}$ the saturated hydraulic conductivity. The parameters $s_{1..4}$ are the coefficients for the hydrological model.
</small>
![](images/table1.png)
...
![](images/fig2.png)
<small>
**Fig. S2.2**. Comparisons of ground heat flux (a) and soil surface temperature (b, c) estimates derived using the full iterative model (true) and using the simpler approximation method (estimated). In (b) over-laid time-series are shown, with the full method depicted by red lines and simpler method shown as grey lines. In (c) estimates derived using the two methods are plotted against one another. In both (a) and (c), the red lines are the 1:1 relationship.
</small>
...
![](images/fig3.png)
<small>
**Fig. S2.3**. Predictions of soil temperature made using the multi-layer soil model included in NicheMapR (red) and the simpler rolling-mean approach implemented in microclimf (black).
</small>
...
![](images/fig4.png)
<small>
**Fig. S2.4**. Comparison of soil moisture (0 – 5 cm depth) estimates derived using the simplified method in microclimf (black) with those obtained using NicheMapR (red). Estimates are for 2017 from Caerthillian Cove in Cornwall, U.K, (49.968°N, 5.215°W) with an assumption of no canopy shading. Data are shown for 11 soil types. Root-Mean-Square (RMS) differences are also indicated. 
</small>
...
![](images/fig5.png)
<small>
**Fig. S2.5**. Example output obtained by spatially distributing soil moisture by topographic wetness with $\theta$ from the point model set to 0.3 and $s=2/3$.Here shown for a 1 x 1 km area at 1m grid resolution centred on Caerthillian Cove in Cornwall, U.K, (49.968°N, 5.215°W). 
</small>
...
The effects of this assumption are shown in Fig. S2.6b (dotted lines).
...
The effects of these assumptions on the temperatures are shown in Fig S2.6b (dashed lines) 
...
![](images/fig6.png)
<small>
**Fig. S2.6.** Comparison between vertical temperature profiles estimated using Raupach’s localised near-field model (red) and the simpler model emulator approach in microclimf (black) here shown for a 10m tall canopy with other vegetation parameters set at their default value, and calculated using an hourly weather dataset from Caerthillian Cove, Lizard, Cornwall (49.968°N, 5.216°W), here shown for a sunny day in May.In(b) modelled and emulated canopy contributions (dotted lines) and combined canopy and ground contibutions (dashed lines) to the far-field contribution are shown. In (c) the combined near- and far-field contributions areshown. In (d) estimates for 5 m above ground are shown for all hours in May (red line = 1:1 relationship).
</small>
...
![](images/fig7.png)
<small>
**Fig. S2.7**. Comparison of snow water equivalent (mm) derived using NicheMapR (red) and microclimf (black) here shown for Caerthillian Cove, Lizard, Cornwall, UK (49.968N, 5.216W) throughout 2017, but with all hourly temperatures reduced by 10°C to ensure the presence of snow.
</small>
...
![](images/fig8.png)
<small>
**Fig. S2.8**. Comparison between the approximate (blue) and full (black) snow model run for Caerthillian Cove, Lizard, Cornwall, UK (49.968N, 5.216W) throughout 2017, but with all hourly temperatures reduced by 8°C to ensure temperatures drop below freezing. Values shown are hourly estimates for the coldest day in each month. In (a) snow depth (cm) is shown and in (b) air temperature 5 cm above ground. Though the two approaches deviate in their estimation of snow depth, the estimation of air temperatures match closely.
</small>
...
Ellers CB, Rowland L, Mencuccini M, Rosas T, Williams K, Harper A, Medlyn BE, Wagner Y, Klein T, Teodoro GS, Oliveira RS, Matos IS, Rosado BHP, Fuchs K, Wohlfahrt G, Montagnan L, Meir P, Sitch S, Cox PM (2020) Stomatal optimization based on xylem hydraulics (SOX)improves land surface model simulation of vegetation responses to climate. *New Phytologist* 226: 1622–37.

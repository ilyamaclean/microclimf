...
are required. To ensure a sensible snow budget the point model is used to calculate hourly melt, and the nature of the terrain and vegetation used to calculate a melt factor by which to multiply snow melt derived from the point model. This quick method is not very reliable however, so below we showcase the slow method, which by default should be used. 

```{r eval=FALSE}
# Run micropoint model
climdata <- microclimf::climdata
climdata$temp <- climdata$temp - 8 # Make it colder so that there is snow
micropoint <- runpointmodel(climdata, reqhgt = 0.05, dtmcaerth, vegp, soilc) 
# Run the snow model using the slow method
smod <- runsnowmodel(climdata, micropoint, vegp, soilc, dtmcaerth, method = "slow")
# Plot mean snow water equivelent and snow depth
...
By default, as is evident from the example above, initial snow depth (and age, which affects snow albedo) is set to zero. Users have the option to set a differetn value, or alternatively to start the model run in summer when there is now snow. In addition to total snow water equivalent, ground snow depth, and average snowpack and ground snow temperature are returned. The micropoint object passed to the snow model is

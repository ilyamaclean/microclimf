...
To model radiation, the canopy as a turbid medium and the transmission of radiation by vegetation is thus described using an equation similar to Beer's law, in which flux density of radiation is assumed dependent on the total one-side leaf area per unit ground area and by an extinction coefficient for the canopy. For direct radiation, the extinction coefficient is assumed to depend on the distribution of leaf angles (with more vertically orientated leaves transmitting less radiation at lower solar altitudes). This is where the model input `vegp$x` comes into play. For isotropic sources of radiation (i.e. diffuse and longwave), leaf angle is assumed unimportant. 
...
vegp2 <- list(pai = aggregate(rast(vegp$x), 10) * 0 + 0.05,
              hgt = aggregate(rast(vegp$hgt), 10) * 0 + 0.005,
              x = aggregate(rast(vegp$x), 10) * 0 + 1,
              gsmax = aggregate(rast(vegp$x), 10) * 0 + 0.1,
              leafr = aggregate(rast(vegp$x), 10) * 0 + 0.3,
              clump = aggregate(rast(vegp$x), 10) * 0,
...
vegp2 <- list(pai = aggregate(rast(vegp$x), 10) * 0 + 3,
...
url <- "https://zenodo.org/records/15009160/files/runmicrobig.zip"
...
The optimal tile size is automatically calculated. Entirely blank tiles (i.e anything with NA in the digital elevation dataset are skipped). Users have the option to specify the tile size or format of the data written to disk and there are quite a few other options for flexibly handling how the model is run. The help file associated with `runmicro_big` gives details. One useful feature is the ability to specify a tile overlap to avoid possible tiling effects. Model output variables for overlapping tiles can then, once converted to `terra::SpatRasters`, can be merged using function `mosaicblend`. This applies a distance weighting to the overlapping area so that sharp boundary effects are avoided. Another potentially useful option is to write the model outputs as compressed nc files. 

In the second example, using `runmicro_biga`, 1km resolution gridded climate data are used to drive the model. It is assumed that data have already been downloaded and unzipped. Again, the the code first runs the point microclimate model, but here the point model must be run for each of the ~400 1km grid cells, which itself takes a little while. Again, the relevant terrain variables are then calculated over the whole study area and the model is then run in tiles. As with `runmicro_big`, the size of tiles and the degree of overlap can be specified as user inputs. 
...
plot(bioclim[[12]], col = rev(mypal)) # BIO2 (in this case soil moisture)
```
![](images/image11.png)\
...
are required. To ensure a sensible snow budget the point model is used to calculate hourly melt, and the nature of the terrain and vegetation used to calculate a melt factor by which to multiply snow melt derived from the point model. Below we compare the two methods

```{r eval=FALSE}
# Run micropoint model
climdata <- microclimf::climdata
climdata$temp <- climdata$temp - 12 # Make it colder so that there is snow
micropoint <- runpointmodel(climdata, reqhgt = 0.05, dtmcaerth, vegp, soilc) 
# Subset point model
micropoint <- subsetpointmodel(micropoint, tstep = "month", what = "tmin")
# Run the snow model in slow mode
smod1 <- runsnowmodel(climdata, micropoint, vegp, soilc, dtmcaerth, method = "slow")
# Run the snow model in quick mode
smod2 <- runsnowmodel(climdata, micropoint, vegp, soilc, dtmcaerth, method = "fast")
# Compute mean time series and plot : depth = snow water equivelent / snow density
sdepc1 <- apply(smod1$totalSWE / smod1$snowden, 3, "mean", na.rm = TRUE)
sdepc2 <- apply(smod2$totalSWE / smod2$snowden, 3, "mean", na.rm = TRUE)
plot(sdepc1, type = "l", col = "blue", ylim = c(0, 1))
par(new = TRUE)
plot(sdepc2, type = "l", col = "red", ylim = c(0, 1))
```
![](images/image14p.png)\

The quick method produces broadly similar results, but as expected, the quick method is far from perfect, particularly in picking-up spatial patterns. Below therefore we showcase the slow method, which by default sis better to use. 
...
smod <- runsnowmodel(climdata, micropoint, vegp, soilc, dtmcaerth, method = "slow", snowenv = "Maritime")
# Plot mean snow water equivalent and snow depth
swe <- apply(smod$totalSWE, 3, mean, na.rm = TRUE) 
depth <- apply(smod$totalSWE / smod$snowden, 3, mean, na.rm = TRUE)  # Depth (m)
par(mfrow = c(2, 1))
plot(swe ~ as.POSIXct(climdata$obs_time), type = "l", 
     xlab = "Month",
     ylab = "Snow water equivalent (mm)")
plot(depth ~ as.POSIXct(climdata$obs_time), type = "l",
     xlab = "Month",
     ylab = "Snow depth (m)")
```
![](images/image14a.png)\

By default, as is evident from the example above, initial snow depth (and age, which affects snow albedo) is set to zero. Users have the option to set a different value, or alternatively to start the model run in summer when there is now snow. In addition to total snow water equivalent, ground snow depth, and average snowpack and ground snow temperature and snow density are returned. You may also have noticed, that in running the example, the snow environment has been switched form the default (`Taiga`) to a more sensible `Maritime`. Other options are `Prairie`, `Alpine` or `Tundra`. This affects the way that snow density evolves as a function of snow age. The micropoint object passed to the snow model is not used to run the snow model itself - it is just used to automatically subset the outputs if necessary when method = `slow` so that the correct data are passed to the microclimate model. We show this below, where we run the microclimate model with and without snow and show the effects that snow has on average temperature and soil moisture. If `reqhgt` is below the snow pack, snow temperature as opposed to air temperature is returned. 
...
micropoint <- subsetpointmodel(micropoint, tstep = "month", what = "tmin")
smod <- runsnowmodel(climdata, micropoint, vegp, soilc, dtmcaerth, method = "slow", snowenv = "Maritime")
# Run microclimate model with snow
mout1 <- runmicro(micropoint, reqhgt = 0.05, vegp, soilc, dtmcaerth, snow = TRUE, snowmod = smod)
# Run microclimate model without snow
mout2 <- runmicro(micropoint, reqhgt = 0.05, vegp, soilc, dtmcaerth) # no snow assumed
Tz1 <- apply(mout1$Tz, 3, mean, na.rm = TRUE)
Tz2 <- apply(mout2$Tz, 3, mean, na.rm = TRUE)
soilm1 <- apply(mout1$soilm, 3, mean, na.rm = TRUE)
soilm2 <- apply(mout2$soilm, 3, mean, na.rm = TRUE)
par(mfrow = c(2, 1))
plot(Tz1, type = "l", ylab = "Temperature", ylim = c(-10, 30), col = "blue")
par(new = TRUE)
plot(Tz2, type = "l", ylab = "Temperature", ylim = c(-10, 30), col = "red")
plot(soilm1, type = "l", ylab = "Soil moisture", ylim = c(0, 0.5), col = "blue")
par(new = TRUE)     
plot(soilm2, type = "l", ylab = "Soil moisture", ylim = c(0, 0.5), col = "red")
```
![](images/image14b.png)\

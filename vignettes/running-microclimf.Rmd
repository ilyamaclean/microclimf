...
The locations with the highest maximum temperatures are those where the vegetation surface lies just above or at 5cm. Heat absorbed by the vegetation and the air warmed significantly in proximity to the vegetation. The locations with the lowest maximum temperatures are those shaded topographically or by vegetation.
...
plot(rast(vegp$leafr),col=gray(0:255/255), main = "Leaf reflectivity")
plot(rast(vegp$clump), main = "Canopy clumping factor") # set to 0 
...
* The `vegp` dataset is a list of the following:
    ...
    * `clump` - a PackedSpatRast object of values between 0 and 1 indicating the degree of canopy clumpiness (0 - even, 1 = highly clumped)
    * `leafd` - a PackedSpatRast object of leaf diameters (m)
    
The inbuilt raster layers in microclimf are stored as PackedSpatRast objects to use less memory (see ?terra::wrap). To convert them to normal SpatRast objects the rast function is used as follows mySpatRast<-rast(myPackedSpatRast). In all instances, the model can be run with objects stored in either format. However, to plot a PackedSpatRast object, one must first unpack it. 
...
Here, the red lines are values form NicheMapR and the black  lines those from the simple model. As can be seen, the simple two-layer soil model approximates near surface soil moisture relatively well.
...
The key step in computing the sensible heat flux is thus computing wind speeds. In `microclimf`, the effects of terrain on wind speed is also accounted for by applying a topographic shelter coefficient. Wind calculations are performed using function `wind`. Since the wind profile at any given location may not just depend on the surface roughness at a point location, but also on surface upwind, the function contains an optional input parameter `xyf` controlling the number of pixels, and hence the distance over which surface roughness is averaged. However, the default is not to average the surface roughness. The degree of averaging has a strong influence on the outputs of the model. In future versions of the model, it is anticipated that the degree of smoothing will be handled automatically.

In the example below wind speed is calculated for 5 cm above ground. The default of no smoothing is assumed, so `xyf` is not specified.

```{r eval=FALSE}
rm(list = ls()) # so that inbuilt vegp etc are used
library(microclimf)
micro<-modelin(climdata,rainfall,vegp,soilc,dtmcaerth)
micro<-wind(micro, reqhgt = 0.05, xyf = 10)
mypal <- colorRampPalette(c("darkblue","blue","green","yellow","orange","red"))(255)
